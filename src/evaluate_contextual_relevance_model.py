...
def find_best_threshold(X_test, community, y_pred, y_test):
    ...
    best_experiment_data['% High recall candidates filtered out'] = get_filtered_out_num(X_test['hard_y_pred'])

    return best_experiment_data

def get_filtered_out_num(hard_y_pred):
    hard_y_pred_value_counts = hard_y_pred.value_counts()
    if 0 not in hard_y_pred_value_counts.index:
        return 0
    return round(hard_y_pred_value_counts.loc[0] / hard_y_pred_value_counts.sum(), 2)
...
def get_measures_for_y_test_and_hard_y_pred(hard_y_pred, y_pred, y_test):
    ...
    return acc, cm, f1_score_score, precision_val, recall_score_score
