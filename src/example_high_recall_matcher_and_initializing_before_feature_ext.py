...
from contextual_relevance.extract_dataset_with_feats.initialize_training_dataset import WindowsMaker
from utils import words_similarity, word_is_english
...
def main():
    ...
    print(f"high_recall_matcher_found: {high_recall_matcher_found}")
    post_df['matches_found'] = [json.dumps(high_recall_matcher_found)]

    # Contextual relevance model - feature extraction
    ## 1 - initialize
    windows_maker = WindowsMaker()
    post_df_initialized = windows_maker.go(post_df)
    print(f"post_df_initialized:")
    print(post_df_initialized)
...
def find_occs_offsets_in_txt(cand_match_occ_in_key_txt, cand_term, post_txt):
    ...
    for i in range(len(txt_words)):
        ...
        if len(relevant_words) > 1 and (relevant_words[1] == '' or relevant_words[1] == ' ') and len(
                txt_words) > i + cand_match_occ_in_key_txt_len:
            relevant_words.append(txt_words[i + cand_match_occ_in_key_txt_len])
        relevant_term = " ".join(relevant_words)
        if words_similarity(relevant_term, cand_term) > 0.82 or words_similarity(relevant_term,
                                                                                 cand_match_occ_in_key_txt) > 0.82:
...
def add_eng_umls_data(eng_matches, umls_data, row):
    ...
    for match_d in eng_matches:
        idx_of_match_in_df = \
        umls_data[umls_data[STRING_COLUMN].apply(lambda x: x.lower() == match_d['umls_match'])].index[0]
        match_cui = umls_data.iloc[idx_of_match_in_df]['CUI']
        match_tui = umls_data.iloc[idx_of_match_in_df]['TUI']
        semantic_type = get_semantic_type(match_tui)
        eng_d = {'cand_match': match_d['cand_match'], 'umls_match': match_d['umls_match'], 'sim': match_d['sim'],
                 'cui': match_cui, 'match_tui': match_tui, 'semantic_type': semantic_type,
                 'all_match_occ': match_d['all_match_occ'],
                 'curr_occurence_offset': match_d['curr_occurence_offset']}
...
def get_eng_text(msg_tokenized_txt):
    ...
    msg_eng_text = " ".join(
        [w.lower() for w in msg_words if w != "" and w != " " and len(w) > 2 and word_is_english(w)])
    return msg_eng_text
