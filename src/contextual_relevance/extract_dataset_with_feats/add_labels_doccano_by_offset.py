...
def handle_community(community):
    ...
    labels_df = labels_df[labels_df[FINAL_LABELS_COL].apply(lambda lst: len(lst) > 0)]
    ...
    for r_idx, row in relevant_feats_df.iterrows():
        ...
        # if row['file_name'] == file_name_to_debug:
        #     print("Yes")
        # else:
        #     continue

        label_row_tokenized_txt = replace_puncs(label_row['tokenized_text'])
        label_row_tokenized_text_single_spaces = " ".join([x for x in label_row_tokenized_txt.split(" ") if x != ' ' and x != ''])
        row_tokenized_text_single_spaces = " ".join([x for x in replace_puncs(row['tokenized_text']).split(" ") if x != ' ' and x != ''])
        ...
        if yi == 1:
            ...
            for m in matches_for_post:
                if m['best_match']['term'] == best_match['term'] and spans_match(m['best_match'], row_start_offset, row_end_offset):
                    not_intersecting = False
            if not_intersecting:
                high_recall_matches_matched_for_file_name[row['file_name']].append({'cand_match': row['cand_match'], 'best_match': best_match})
    ...
    for label_row_idx, label_row in labels_df.iterrows():
        ...
        unmatched_labels = leave_only_unmatched_labels(matches_for_filename, label_row[FINAL_LABELS_COL], label_row['file_name'])
        high_recall_matches_lst = get_high_recall_matches_lst_for_post(label_row, relevant_feats_df)
        # print("labels", unmatched_labels)
        # print("matches", high_recall_matches_lst)
    ...
    print(f"all_labeled_terms_without_matches: {num_all_labeled_terms_without_matches}, len set: {num_set_all_labeled_terms_without_matches}")
    good_case_fns = num_all_labeled_terms_without_matches - could_be_matched
    print(f"Community: {community}, FNs: {num_all_labeled_terms_without_matches}, could be matched: {could_be_matched} => {good_case_fns}, 25% of curr: {num_all_labeled_terms_without_matches * 0.25}")
...
if __name__ == '__main__':
    ...
    print(f"Global missed_labels_counter:")
