...
def handle_community(community):
    ...
    for r_idx, row in relevant_feats_df.iterrows():
        ...
        if row['cand_match'] in relevant_labeled_df[final_label_col] or row['umls_match'] in relevant_labeled_df[final_label_col]:
            all_labels.append(1)
        else:
            best_match, best_match_sim = get_best_match(relevant_labeled_df, row, final_label_col)
            if best_match:
...
def get_data_from_annotation_file(comm_lines, posts_df):
    ...
    for line_idx, line in enumerate(comm_lines):
        ...
        tokenized_text_no_dots = additional_post_data['tokenized_text'].replace(".", " ")
